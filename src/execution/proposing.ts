...
const calcSafeTxHash = async (
  ...
  if (onChainHash != offChainHash)
    throw Error("Unexpected hash! (For pre-1.3.0 version use --on-chain-hash)");
  return offChainHash;
};

export { calcSafeTxHash };
...
task("propose", "Create a Safe tx proposal json file")
  ...
  .addFlag(
    "delegatecall",
    "Indicator if tx should be executed as a delegatecall"
  )
  .addFlag(
    "onChainHash",
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (!isHexString(taskArgs.data))
      throw Error(`Invalid hex string provided for data: ${taskArgs.data}`);
    const tx = buildSafeTransaction({
      to: taskArgs.to,
      value: parseEther(taskArgs.value).toString(),
      data: taskArgs.data,
...
const buildMetaTx = (description: TxDescription): MetaTransaction => {
  ...
  const data = isHexString(description.data)
    ? description.data!!
    : description.method
    ? buildData(description.method, description.params)
    : "0x";
...
const parseMultiSendJsonFile = async (
...
task("propose-multi", "Create a Safe tx proposal json file")
  ...
  .addPositionalParam(
    "txs",
    "Json file with transactions",
    undefined,
    types.inputFile
  )
  .addParam(
    "multiSend",
    "Set to overwrite which multiSend address to use",
    "",
    types.string,
    true
  )
  .addParam(
    "nonce",
    "Set nonce to use (will default to on-chain nonce)",
    "",
    types.string,
    true
  )
  ...
  .addParam(
    "name",
    "Name to be used for the transaction builder json",
    undefined,
    types.string,
    true
  )
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (taskArgs.export) {
      await writeTxBuilderJson(
        ...
        chainId.toString(),
        txs,
        taskArgs.name || "Custom Transactions"
      );
      return;
    }
    const tx = await parseMultiSendJsonFile(
      hre,
      txs,
